...
export async function addTaskData(task: TaskData): Promise<void> {
  try {
    const response = await fetch('http://localhost:3001/api/tasks', {
      ...
      body: JSON.stringify([task]), // Enviando como array de uma tarefa
    });
...
export async function getGamificationTasks(): Promise<Task[]> {
  try {
    const response = await fetch('http://localhost:3001/api/gamification-tasks');
    if (!response.ok) {
      throw new Error(`Erro na requisição: ${response.status}`);
    }
    const tasks = await response.json();
...
export async function getUserMissions(userId: string): Promise<ActiveMission[]> {
  try {
    const response = await fetch(`http://localhost:3001/api/user-missions/${userId}`);
    if (!response.ok) {
      throw new Error(`Erro na requisição: ${response.status}`);
    }
