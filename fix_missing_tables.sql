-- Script completo para corrigir todas as tabelas faltantes no Supabase
-- Execute este script no SQL Editor do Supabase

-- ============================================================================
-- 1. CORREÇÃO DA TABELA USER_PREFERENCES (adicionar coluna preferences)
-- ============================================================================
...
-- Tabela para log de acesso dos usuários (substitui user_access_log)
...
-- Tabela para configuração de streaks (usa gamification_config como alternativa)
-- Mas vamos criar uma tabela específica se necessário
...
-- Tabela para configuração de temporada (usa season_rankings como alternativa)
-- Mas vamos criar uma tabela específica se necessário
...
-- Tabela para estados da aplicação do usuário (para sincronização)
...
-- Políticas para user_access_log
...
-- Políticas para streak_config (configurações globais)
...
-- Políticas para season_config (configurações globais)
...
-- Políticas para user_app_states
...
-- Inserir configuração padrão de streak
...
-- Verificar se a coluna preferences foi adicionada
DO $$
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Coluna preferences não foi adicionada à tabela user_preferences!';
    END IF;

    RAISE NOTICE '✅ Coluna preferences adicionada com sucesso!';
END $$;
...
-- Comentários finais
COMMENT ON TABLE user_access_log IS 'Log de acessos dos usuários ao sistema';
COMMENT ON TABLE streak_config IS 'Configurações do sistema de streaks';
COMMENT ON TABLE streak_awards IS 'Prêmios de streak concedidos aos usuários';
COMMENT ON TABLE season_config IS 'Configurações das temporadas do jogo';
COMMENT ON TABLE user_app_states IS 'Estados da aplicação salvos por usuário';